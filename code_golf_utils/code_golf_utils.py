...
Portable version: works on Kaggle and locally (VS Code).
"""
...
# --- portability flags/paths -------------------------------------------------
IS_KAGGLE = os.path.exists("/kaggle/input")
# Let env override the local root if you move the project.
ROOT_LOCAL = os.getenv("GOLF_ROOT", r"C:\Users\EDDY\sirharginger\google-code-golf-2025")

# Location of the JSON tasks (ARC-AGI and ARC-GEN).
# Kaggle:  /kaggle/input/google-code-golf-2025/
# Local :  <ROOT_LOCAL>/data/
code_golf_dir = (
    "/kaggle/input/google-code-golf-2025/"
    if IS_KAGGLE
    else os.path.join(ROOT_LOCAL, "data") + os.sep
)
...
def load_examples(task_num):
  ...
  with open(os.path.join(code_golf_dir, f"task{task_num:03d}.json")) as f:
...
def verify_program(task_num, examples, task_path=None):
  ...
  If task_path is None:
    - Kaggle: uses /kaggle/working/task.py
    - Local : uses <ROOT_LOCAL>/code/taskNNN.py
  """
  task_name = "task_with_imports"
  if task_path is None:
    task_path = ("/kaggle/working/task.py" if IS_KAGGLE
                 else os.path.join(ROOT_LOCAL, "code", f"task{task_num:03}.py"))
